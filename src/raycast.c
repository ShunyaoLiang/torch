	...
	float end_slope)
{
	...
	for (; dx <= octant->dx_max && !blocked; ++dx) {
		...
		struct tile *tile = get_transformed_tile_and_call_back(raycast, dx, dy, octant->number);

		blocked = tile_blocks_light(tile);

		for (++dy; dy <= dy_interval.end; ++dy) {
			tile = get_transformed_tile_and_call_back(raycast, dx, dy, octant->number);
			...
			if (blocked && !tile_blocks_light(tile)) {
				...
				blocked = true;
				raycast_octant_at(raycast, octant, dx + 1, start_slope, (dy - 0.5f) / (dx + 0.5f));
			}
		}
	}
}
